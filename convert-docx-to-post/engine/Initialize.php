...
use Convert_Docx_To_Post\Engine;
...
class Initialize {
	...
	private function load_classes() {
		$this->classes = \apply_filters( 'convert_docx_to_post_classes_to_execute', $this->classes );
		...
		foreach ( $this->classes as $class ) {
			try {
				...
				\do_action( 'convert_docx_to_post_initialize_failed', $err );
	...
	private function get_classes( string $namespace ) {
		...
		$namespace = 'Convert_Docx_To_Post\\' . $namespace;

		// In case composer has autoload optimized
		if ( isset( $classmap[ 'Convert_Docx_To_Post\\Engine\\Initialize' ] ) ) {
